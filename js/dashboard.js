    ...
    if (user) {
        ...
        try {
            ...
            if (snap.exists()) {
                ...
                const role = userData.role; // OWNER, MANAGER, atau WORKER
                
                document.getElementById('userRole').innerText = role;

                // PERBAIKAN: Sekarang me-render semua role termasuk WORKER
...
// --- 2. RENDER AREA PANEL KHUSUS (PERBAIKAN DI SINI) ---
function renderAdminArea(role) {
    ...
    if (role === 'OWNER') {
        html = `
            <div class="p-4 glass rounded-3xl border border-yellow-500/30 bg-yellow-500/5 flex justify-between items-center mb-6 animate-pulse">
                <div class="text-left">
        ...
        // TAMBAHAN: Tombol khusus buat WORKER biar nggak kayak Buyer
        html = `
            <div class="p-4 glass rounded-3xl border border-blue-500/30 bg-blue-500/5 flex justify-between items-center mb-6">
                <div class="text-left">
                ...
                <button onclick="window.location.href='workerpanel.html'" class="bg-blue-600 px-4 py-2 rounded-xl text-[10px] font-black shadow-lg shadow-blue-900/20 active:scale-95 transition-all">BUKA PANEL</button>
            </div>`;
    }
    area.innerHTML = html;
}
...
async function loadNavCategories() {
    ...
    let html = `<button onclick="filterService('all')" class="cat-btn active px-6 py-2.5 rounded-2xl text-[10px] font-black border border-gray-800 transition-all whitespace-nowrap bg-[#1a1a1a]/50">SEMUA</button>`;
...
function renderProducts(data) {
    ...
    container.innerHTML = data.map(p => `
        <div class="glass rounded-[2rem] overflow-hidden border border-gray-800/50 hover:border-blue-500/50 transition-all duration-500 group flex flex-col h-full cursor-pointer" onclick="window.location.href='product-detail.html?id=${p.id}'">
            <div class="h-40 w-full bg-cover bg-center transition-transform group-hover:scale-105 duration-700" style="background-image: url('${p.images ? p.images[0] : ''}')"></div>
            <div class="p-5 flex-1 flex flex-col">
                ...
                <button class="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-900/20">BELI SEKARANG</button>
            </div>
        </div>`).join('');
}
